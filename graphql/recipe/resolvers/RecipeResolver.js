...
const userCollection_1 = __importDefault(
  require('../../../models/userCollection')
);
const blendIngredient_1 = __importDefault(
  require('../../../models/blendIngredient')
);
const userNote_1 = __importDefault(require('../../../models/userNote'));
const RecipeVersionModel_1 = __importDefault(
  require('../../../models/RecipeVersionModel')
);
const scrappedRecipe_1 = __importDefault(
  require('../../../models/scrappedRecipe')
);
const Recipe_1 = __importDefault(require('../../recipe/schemas/Recipe'));
const SimpleRecipe_1 = __importDefault(
  require('../../recipe/schemas/SimpleRecipe')
);
...
const GetAllRecipesByBlendCategory_1 = __importDefault(
  require('./input-type/GetAllRecipesByBlendCategory')
);
const Compare_1 = __importDefault(require('../../../models/Compare'));
const Hello_1 = __importDefault(require('../schemas/Hello'));
const updateVersionFacts_1 = __importDefault(
  require('./util/updateVersionFacts')
);
...
const CreateScrappedRecipe_1 = __importDefault(
  require('./input-type/CreateScrappedRecipe')
);
const RecipesWithPagination_1 = __importDefault(
  require('../schemas/RecipesWithPagination')
);
const UserRecipeProfile_1 = __importDefault(
  require('../../../models/UserRecipeProfile')
);
const getNotesCompareAndUserCollection_1 = __importDefault(
  require('./util/getNotesCompareAndUserCollection')
);
...
const planCollection_1 = __importDefault(
  require('../../../models/planCollection')
);
const temporaryCompareCollection_1 = __importDefault(
  require('../../../models/temporaryCompareCollection')
);
const changeCompare_1 = __importDefault(
  require('../../member/resolvers/util/changeCompare')
);
const GetASingleRecipe_1 = __importDefault(require('./util/GetASingleRecipe'));
const ProfileRecipeDesc_1 = __importDefault(
  require('../schemas/ProfileRecipeDesc')
);
const Collection_1 = __importDefault(
  require('../../member/schemas/Collection')
);
const planCollection_2 = __importDefault(
  require('../../../models/planCollection')
);
const challenge_1 = __importDefault(require('../../../models/challenge'));
const ChallengePost_1 = __importDefault(
  require('../../../models/ChallengePost')
);
const InviteForChallenge_1 = __importDefault(
  require('../../../models/InviteForChallenge')
);
...
const collectionShare_1 = __importDefault(
  require('../../../models/collectionShare')
);
const shareChallengeGlobal_1 = __importDefault(
  require('../../../models/shareChallengeGlobal')
);
const share_1 = __importDefault(require('../../../models/share'));
const comment_1 = __importDefault(require('../../../models/comment'));
const adminCollection_1 = __importDefault(
  require('../../../models/adminCollection')
);
...
let RecipeResolver = class RecipeResolver {
   ...
   */
  async tya() {
    let recipeVersions = await RecipeVersionModel_1.default
      .find({
        gigl: null,
      })
      .select('_id');
  ...
  async getCompareList(userId) {
    const compareList = await Compare_1.default
      .find({ userId: userId })
      .populate({
        path: 'recipeId',
        model: 'RecipeModel',
    ...
    let memberCollection = await memberModel_1.default
      .findOne({ _id: userId })
      .populate({
        path: 'collections',
  ...
  async getAllRecipesByBlendCategory(data) {
    ...
    if (data.includeIngredientIds.length > 0) {
      recipes = await recipeModel_1.default
        .find({
          global: true,
          userId: null,
          addedByAdmin: true,
  ...
  async getAllRecipes(userId) {
    if (userId) {
      const recipes = await recipeModel_1.default
        .find({})
        .populate({
          path: 'ingredients.ingredientId',
          model: 'BlendIngredient',
      ...
      let returnRecipe = [];
      let collectionRecipes = [];
      let memberCollections = await memberModel_1.default
        .find({ _id: userId })
        .populate({
          path: 'collections',
          model: 'UserCollection',
  ...
  async searchRecipes(searchTerm, userId, page, limit) {
    ...
    let recipes = await recipeModel_1.default
      .find({
        // global: true,
        // userId: null,
        // addedByAdmin: true,
    ...
    let userProfileRecipes = await UserRecipeProfile_1.default
      .find({
        userId: userId,
        recipeId: {
          $in: recipeIds,
      ...
      .populate({
        ...
        select:
          'mainEntityOfPage name image datePublished recipeBlendCategory brand foodCategories url favicon numberOfRating totalViews averageRating userId',
      })
      .populate({
        path: 'defaultVersion',
        model: 'RecipeVersion',
      ...
      .limit(limit)
      .skip(limit * (page - 1));
    let returnRecipe = await (0, getNotesCompareAndUserCollection_1.default)(
      userId,
      userProfileRecipes
    );
    let userProfileRecipesTotalCount =
      await UserRecipeProfile_1.default.countDocuments({
        userId: userId,
        recipeId: {
          $in: recipeIds,
        },
  ...
  async getARecipe(recipeId, userId, token) {
    if (token) {
      if (!userId) {
        ...
        let data = await (0, util_1.default)(
          token.toString(),
          userId.toString()
        );
        if (!data) {
    ...
    const recipe = await recipeModel_1.default
      .findById(recipeId)
      .populate({
        path: 'ingredients.ingredientId',
        model: 'BlendIngredient',
    ...
    let collectionData = collectionRecipes.filter(
      (recipeData) => recipeData.recipeId === String(recipe._id)
    );
    if (collectionData.length === 0) {
      collectionData = null;
    } else {
  ...
  async deleteARecipe(recipeId, userId) {
    let user = await memberModel_1.default
      .findOne({ _id: userId })
  ...
  async addRecipeFromUser(data, isAddToTemporaryCompareList) {
    ...
    if (data.url) {
      let recipe = await recipeModel_1.default
        .findOne({ url: data.url })
        .select('_id defaultVersion');
      ...
      if (recipe) {
        ...
        if (!isAddToTemporaryCompareList) {
          ...
          if (!recipeCompare) {
            let tempCompareList =
              await temporaryCompareCollection_1.default.findOne({
                userId: data.userId,
                recipeId: recipe._id,
              });
            if (!tempCompareList) {
              ...
              await (0, changeCompare_1.default)(
                String(recipe._id),
                data.userId
              );
            }
        ...
        return await (0, GetASingleRecipe_1.default)(
          String(recipe._id),
          String(data.userId),
          null
        );
    ...
    let returnUserRecipe = await recipeModel_1.default
      .findOne({ _id: userRecipe._id })
      .populate('recipeBlendCategory')
      .populate({
        path: 'ingredients.ingredientId',
        model: 'BlendIngredient',
    ...
    if (!isAddToTemporaryCompareList) {
      await userCollection_1.default.findOneAndUpdate(
        { _id: userDefaultCollection },
        { $addToSet: { recipes: returnUserRecipe._id } }
      );
    } else {
      let recipeCompare = await Compare_1.default.findOne({
        recipeId: userRecipe._id,
  ...
  async getAllRecipesBasedOnIngredient(ingredientId) {
    let recipes = await recipeModel_1.default
      .find({
        'ingredients.ingredientId': new mongoose_1.default.mongo.ObjectId(
          ingredientId
        ),
      })
      .populate({
  ...
  async filterRecipe(data, userId, page, limit) {
    ...
    if (recipeData.length > 0 && data.excludeIngredientIds.length > 0) {
      ...
      recipeData = await recipeModel_1.default
        .find({
          _id: { $in: recipeIds },
    ...
    let userRecipes = await UserRecipeProfile_1.default
      .find(findfacts)
    ...
    for (let i = 0; i < data.nutrientFilters.length; i++) {
      let obj = {};
      obj.blendNutrientRefference = new mongoose_1.default.mongo.ObjectId(
        data.nutrientFilters[i].nutrientId.toString()
      );
    ...
    if (energy.length > 0) {
      for (let i = 0; i < energy.length; i++) {
        ...
        recipeFacts = await RecipeVersionModel_1.default
          .find(findfacts)
          .select('recipeId');
  ...
  async makeSomeGlobalRecipes() {
    ...
    for (let i = 0; i < recipes.length; i++) {
      await recipeModel_1.default.updateOne(
        { _id: recipes[i]._id },
        {
          $set: {
            global: true,
  ...
  async resetApplication() {
    ...
    for (let i = 0; i < users.length; i++) {
      let defaultCollection = await userCollection_1.default
        .findOne({
          userId: users[i]._id,
          name: 'My Favorite',
        })
      ...
      let defaultPlanCollection = await planCollection_2.default
        .findOne({
          userId: users[i]._id,
          name: 'My Favorite',
        })
      ...
      await memberModel_1.default.updateOne(
        { _id: users[i]._id },
        {
          $set: {
...
__decorate(
  [
    ...
    __param(1, (0, type_graphql_1.Arg)('userId')),
    __metadata('design:type', Function),
    __metadata('design:paramtypes', [EditRecipe_1.default, String]),
    __metadata('design:returntype', Promise),
  ],
  RecipeResolver.prototype,
  'editARecipe',
...
__decorate(
  [
    ...
    __metadata('design:type', Function),
    __metadata('design:paramtypes', [CreateRecipe_1.default, Boolean]),
    __metadata('design:returntype', Promise),
  ],
  RecipeResolver.prototype,
  'addRecipeFromUser',
...
__decorate(
  [
    ...
    __param(
      0,
      (0, type_graphql_1.Arg)('data', (type) => [
        CreateScrappedRecipe_1.default,
      ])
    ),
    __metadata('design:type', Function),
...
__decorate(
  ...
  RecipeResolver.prototype,
  'getAllRecipesBasedOnIngredient',
  null
);
__decorate(
  [
...
RecipeResolver = __decorate([(0, type_graphql_1.Resolver)()], RecipeResolver);
