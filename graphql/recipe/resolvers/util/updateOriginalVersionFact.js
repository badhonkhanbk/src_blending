...
async function updateOriginalVersionFacts(recipeVersionId) {
    ...
    let versionFact = await recipeOriginalFactModel_1.default.findOne({
    ...
    if (versionFact) {
        await recipeOriginalFactModel_1.default.findOneAndUpdate({ versionId: version._id }, {
        ...
        await recipeOriginalFactModel_1.default.create({
            versionId: version._id,
    ...
    for (let i = 0; i < energyTrees.length; i++) {
        ...
        await recipeOriginalFactModel_1.default.findOneAndUpdate({
            versionId: version._id,
        }, {
            $push: {
...
exports.default = updateOriginalVersionFacts;
...
async function getChildsNutrient(childs, versionId, type) {
    ...
    for (let i = 0; i < childsKey.length; i++) {
        ...
        if (type === 'energy') {
            await recipeOriginalFactModel_1.default.findOneAndUpdate({ versionId: versionId }, {
                $push: {
                    energy: formatedData,
                },
            });
